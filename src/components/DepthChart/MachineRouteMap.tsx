...
// Calculate distance between first and last DEPTH events and highlight them
  useEffect(() => {
    ...
    if (depthEvents.length >= 2) {
      ...
      // Create polyline connecting all DEPTH events
      if (depthPath.length >= 2) {
        ...
        setDepthEventsLine(line);
        
        // Highlight first and last DEPTH events with special markers
        const firstPosition = depthPath[0];
        const lastPosition = depthPath[depthPath.length - 1];
        
        // First DEPTH marker (green)
          ...
          icon: {
            url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                ...
                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" filter="url(#glow)"></path>
                <circle cx="12" cy="10" r="3" fill="#16a34a"></circle>
                <text x="12" y="16" text-anchor="middle" fill="white" font-size="8" font-weight="bold">START</text>
              </svg>
            `)}`,
            scaledSize: new google.maps.Size(48, 48),
            anchor: new google.maps.Point(24, 48)
          },
          title: 'First DEPTH Event'
        });
        setFirstDepthMarker(firstMarker);
        
        // Last DEPTH marker (red)
        const lastMarker = new google.maps.Marker({
          position: lastPosition,
          map,
          icon: {
            url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                <defs>
                  <filter id="glow">
                    <feGaussianBlur stdDeviation="3" result="coloredBlur"/>
                    <feMerge> 
                ...
                <text x="12" y="16" text-anchor="middle" fill="white" font-size="8" font-weight="bold">END</text>
              </svg>
            `)}`,
            scaledSize: new google.maps.Size(48, 48),
            anchor: new google.maps.Point(24, 48)
          },
          title: 'Last DEPTH Event'
        });
        setLastDepthMarker(lastMarker);
      }
    } else {
