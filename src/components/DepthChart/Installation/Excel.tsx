...
export interface GPInstallationRow {
  ...
  key_person?: KeyPerson | string;
  RFMS_FILTERS?: SmartRack[] | string;
  created_at?: string;
  updated_at?: string;
}
...
export interface BlockInstallationRow {
  ...
  block_code?: string;
  block_name?: string;
  block_latitude?: string | number;
  block_longitude?: string | number;
...
function sharedEquipmentHeaders(): string[] {
  return [
    'Smart Rack - Make',
    'Smart Rack - Serial No',
    'Smart Rack - Type',
    'Smart Rack - Photo',
    'FDMS Shelf - Make',
    'FDMS Shelf - Serial No',
    'FDMS Shelf - Count',
    'FDMS Shelf - Front Photo',
    'FDMS Shelf - Left Photo',
    'FDMS Shelf - Right Photo',
    'FDMS Shelf - QR Photo',
    'RFMS Filter - Make',
    'RFMS Filter - Serial No',
    'RFMS Filter - Type',
    'RFMS Filter - Photo',
    'IP MPLS Router - Make',
    'IP MPLS Router - Serial No',
    'IP MPLS Router - Type',
    'IP MPLS Router - Photo',
    'SFP 10G/40 - Make',
    'SFP 10G/40 - Serial No',
    'SFP 10G/40 - Count',
    'SFP 10G/40 - Photo',
    'SFP 1G/10 - Make',
    'SFP 1G/10 - Serial No',
...
function sharedEquipmentCells(
  ...
  return [
    ...
    fdms.serial_no ?? '',
    fdms.count ?? '',
    photoLink(fdms.front_photo),
    photoLink(fdms.left_photo),
    photoLink(fdms.right_photo),
    photoLink(fdms.qr_photo),
    rf?.make ?? '',
    rf.serial_no ?? '',
    rf.type ?? '',
    photoLink(rf.photo),
...
const GP_HEADERS: string[] = [
  ...
  'Power MPPT - Available',
  'Power MPPT - Make',
  'Power MPPT - Serial No',
  'Power MPPT - Battery SNO',
  'Power MPPT - Solar Panel SNO',
  'Power MPPT - UPS SNO',
  'Power MPPT - Photo',
  'Power w/o MPPT - Available',
  'Power w/o MPPT - Make',
  'Power w/o MPPT - Serial No',
  'Power w/o MPPT - Battery SNO',
  'Power w/o MPPT - Solar Panel SNO',
  'Power w/o MPPT - UPS SNO',
  'Power w/o MPPT - Photo',
  'Solar 1KW - Make',
  'Solar 1KW - Serial No',
  'Solar 1KW - Battery SNO',
  'Solar 1KW - Solar Panel SNO',
  'Solar 1KW - UPS SNO',
  'Solar 1KW - Photo',
  'Equipment Photo 1',
  'Equipment Photo 2',
...
function flattenGPRow(row: GPInstallationRow): CellValue[] {
  ...
  return [
    ...
    pmppt.serial_no ?? '',
    pmppt.battery_sno ?? '',
    pmppt.solar_pannel_sno ?? '',
    pmppt.ups_sno ?? '',
    photoLink(pmppt.photo),
    // Power w/o MPPT
    pwout.available ?? '',
    pwout.make ?? '',
    pwout.serial_no ?? '',
    pwout.battery_sno ?? '',
    pwout.solar_pannel_sno ?? '',
    pwout.ups_sno ?? '',
    photoLink(pwout.photo),
    // Solar 1KW
    solar.make ?? '',
    solar.serial_no ?? '',
    solar.battery_sno ?? '',
    solar.solar_pannel_sno ?? '',
    solar.ups_sno ?? '',
...
function buildWorksheet<T>(
  ...
  ws['!cols'] = headers.map((h) => ({
    wch: Math.min(Math.max(h.length + 2, 14), 40),
  }));
...
export async function exportGPInstallationExcel(
  ...
  try {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      buildWorksheet(filteredData, GP_HEADERS, flattenGPRow),
      'GP Installation',
    );
...
export async function exportBlockInstallationExcel(
  ...
  try {
    ...
    XLSX.utils.book_append_sheet(
      wb,
      buildWorksheet(filteredData, BLOCK_HEADERS, flattenBlockRow),
      'Block Installation',
    );
