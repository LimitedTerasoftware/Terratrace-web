...
const getMachineIcon = (markerColor: string): google.maps.Icon => ({
...
const getPathPointIcon = (markerColor: string): google.maps.Icon => ({
  url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28">
      <circle cx="14" cy="14" r="10" fill="${markerColor}" stroke="#ffffff" stroke-width="3"/>
      <path d="M8 14h12M14 8v12" stroke="#ffffff" stroke-width="2.5" stroke-linecap="round"/>
    </svg>
  `)}`,
  ...
  useEffect(() => {
    ...
    const newMarkers = markers.map((markerData) => {
      ...
      const marker = new google.maps.Marker({
        ...
        icon: markerData.isLatestForMachine
          ? getMachineIcon(markerColor)
          : getPathPointIcon(markerColor),
      });
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-20 right-4 bg-white p-3 rounded-lg shadow-lg border border-gray-200 max-w-xs">
        ...
        <div className="mb-3">
          <h4 className="text-sm font-semibold text-gray-900 mb-2">Points</h4>
        ...
        <div className="mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-700">Latest Position</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#16a34a]"></div>
            <span className="text-xs text-gray-700">Active Path</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#dc2626]"></div>
            <span className="text-xs text-gray-700">Inactive Path</span>
          </div>
        </div>
