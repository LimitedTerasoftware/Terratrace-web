...
function Eventreport() {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'map' | 'chart'>(
    'details',
  );
  ...
  const extractMediaFromRow = (row: Activity): MediaItem[] => {
        ...
        urls.forEach((url, index) => {
          if (url) {
            mediaItems.push({
              ...
              url: `${baseUrl}${url}`,
              label:
                `${labelPrefix} ${urls.length > 1 ? index + 1 : ''}`.trim(),
            });
          }
        });
      }
    ...
    if (
      row.eventType === 'JOINTCHAMBER' ||
      row.eventType === 'MANHOLES' ||
      row.eventType === 'STARTPIT' ||
      row.eventType === 'ENDPIT'
    ) {
    ...
    },
    {
      ...
      cell: (row: Activity) => {
        ...
        if (typeof row.videoDetails === 'string') {
          parsedVideoDetails = row?.videoDetails
            ? JSON.parse(row?.videoDetails)
            : null;

          const videoUrl = parsedVideoDetails?.videoUrl
            ?.trim()
            .replace(/^"|"$/g, '');
          return videoUrl ? (
            <button
  ...
  const handleExcel = () => {
    ...
    const dataRows = filteredData.map((item) => {
        ...
        if (rawPhotoData.startsWith('[') && rawPhotoData.endsWith(']')) {
          ...
          if (urls.length > 0) {
            imageLinks = urls
              .map(
                (url, i) =>
                  `=HYPERLINK("${baseUrl}${url}", "${item.eventType}_Photo_${i + 1}")`,
              )
              .join(', ');
          }
        } else {
      ...
      return [
        ...
        item.routeFeatureType,
        item.Roadfesibility,
        item.area_type,
        formatPoleForExcel(poleTypeData), // Pole Type
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      'Underground Construcion Details',
    );

    XLSX.writeFile(workbook, 'Underground_Construcion_Details.xlsx', {
      compression: true,
    });
