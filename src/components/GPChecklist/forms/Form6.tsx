...
export default function Form6({ data, onChange }: Form6Props) {
  ...
  const triggerPrint = async () => {
    ...
    printWindow.document
      .write(`<!DOCTYPE html><html><head><title>GP Checklist - Form 6</title>
    ...
    addSection(
      'Site Cleanliness',
      `
      ${addField('Site Clean Status', data?.siteClean)}
      ${materialImages.length > 0 ? `<div class="subsection-title">Site Images</div>${await addImages(materialImages, 'Site Clean')}` : ''}
    `,
    );
    const materialsHtml =
      data?.materialsApproved === 'yes'
        ? data?.verificationProof
          ? typeof data.verificationProof === 'string'
            ? `<div class="file-info">📄 <a href="${data.verificationProof}" target="_blank">View Verification Proof</a></div>`
            : `<div class="file-info">📄 ${(data.verificationProof as File).name}</div>`
          : '<div class="text-value">Materials Approved</div>'
        : addField('Materials Approved', data?.materialsApproved);
    addSection('Materials Verification', materialsHtml);

    const socialHtml =
      data?.socialAudit === 'yes'
        ? data?.socialAuditVideo
          ? typeof data.socialAuditVideo === 'string'
            ? `<div class="file-info">🎥 <a href="${data.socialAuditVideo}" target="_blank">View Social Audit Video</a></div>`
            : `<div class="file-info">🎥 ${(data.socialAuditVideo as File).name}</div>`
          : '<div class="text-value">Social Audit Completed</div>'
        : addField('Social Audit', data?.socialAudit);
    addSection('Social Audit', socialHtml);
    addSection(
      'Site Label Board',
      `
      ${addField('Site Label Board Installed', data?.siteLabelBoard)}
      ${data?.siteLabelBoard === 'yes' && siteLabelBoardImages.length > 0 ? `<div class="subsection-title">Images</div>${await addImages(siteLabelBoardImages, 'Site Label Board')}` : ''}
    `,
    );

    const content = buildPrintPage(
      'GP Checklist — Safe Quality Verification',
      'Form 6 — Safety, Quality & Social Audit Verification',
      sections.join(''),
      attachmentPages,
    );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto">
          <button
            onClick={handlePrint}
            disabled={preparing}
            className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-indigo-200 text-indigo-700 rounded-lg hover:bg-indigo-50 transition-colors text-sm font-medium disabled:opacity-50"
          >
            {preparing ? (
