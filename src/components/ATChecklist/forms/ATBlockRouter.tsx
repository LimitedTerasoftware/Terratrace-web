...
export { DOC_CODE } from '../print/atBlockRouterPrint';
...
const ATBlockRouterForm = ({
  ...
  const toPrintFile = async (
    ...
    const kind = isImageFile(slot.file, slot.url) ? 'image' : isPdfFile(slot.file, slot.url) ? 'pdf' : 'other';
    try {
      if (kind === 'image') {
  ...
  const toPrintFiles = async (
    item: { images: UploadedFile[]; documents: UploadedFile[] },
    labelPrefix: string,
  ): Promise<PrintFile[]> => {
    const files = await Promise.all(
      [...item.images, ...item.documents].map((file, index) =>
        toPrintFile(file, `${labelPrefix}-${index + 1}`),
      ),
    );
  ...
  const triggerPrint = async () => {
    ...
    for (const item of basicItems) {
      basic[item.id] = {
        compliance: item.compliance,
        remarks: item.remarks,
        files: await toPrintFiles(item, item.testCaseNo),
      };
    }
    ...
    for (const item of networkItems) {
      network[item.id] = {
        ...
        testResults: item.testResults,
        remarks: item.remarks,
        files: await toPrintFiles(item, `Test ${item.testNo}`),
      };
    }
    ...
    const html = buildATBlockRouterPrintHtml({
      ...
      networkDiagram: await toPrintFile(networkDiagramFile, 'Block network diagram'),
      qrCode: await toPrintFile(qrCodeFile, 'QR code'),
      certifications,
      basic,
      network,
