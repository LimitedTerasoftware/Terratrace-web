...
export const PLACEMARK_CATEGORIES: Record<
  string,
...
> = {
  ...
  // Construction Data Categories
  'Construction: Depth': { color: '#059669', icon: '📏' },
  'Construction: Start Pit': { color: '#0891B2', icon: '🕳️' },
  'Construction: End Pit': { color: '#DC2626', icon: '🔴' },
  ...
  'External O & M: Survey: Block to FPOI Cable': {
    color: '#FF6B35',
    icon: '🔗',
  }, // Orange-red
...
function extractSurveyImages(point: any): SurveyImage[] {
  ...
  if (
    !point.surveyUploaded ||
    point.surveyUploaded === '' ||
    point.surveyUploaded === 'false'
  ) {
    return images;
  }
  ...
  try {
    ...
    try {
      const startPhotos = point.start_photos
        ? JSON.parse(point.start_photos)
        : [];
    ...
    if (
      point.landMarkUrls &&
      point.event_type === 'LANDMARK' &&
      point.landMarkType !== 'NONE'
    ) {
      try {
        const parsed =
          typeof point.landMarkUrls === 'string'
    ...
    if (point.event_type === 'ROADCROSSING' && point.road_crossing) {
      try {
        const rc =
          typeof point.road_crossing === 'string'
        ...
        if (rc.startPhoto) {
          ...
          if (isValidImageUrl(resolvedUrl)) {
            images.push({
              ...
              coordinates:
                rc.startPhotoLat && rc.startPhotoLong
                  ? {
                      lat: parseFloat(rc.startPhotoLat),
                      lng: parseFloat(rc.startPhotoLong),
                    }
                  : point.latitude && point.longitude
                    ? {
                        lat: parseFloat(point.latitude),
        ...
        if (rc.endPhoto) {
          ...
          if (isValidImageUrl(resolvedUrl)) {
            images.push({
              ...
              coordinates:
                rc.endPhotoLat && rc.endPhotoLong
                  ? {
                      lat: parseFloat(rc.endPhotoLat),
                      lng: parseFloat(rc.endPhotoLong),
                    }
                  : point.latitude && point.longitude
                    ? {
                        lat: parseFloat(point.latitude),
    ...
    const photoFields = [
      ...
      {
        field: 'jointChamberUrl',
        eventType: 'JOINTCHAMBER',
        type: 'joint_chamber',
        label: 'Joint Chamber Photo',
      },
      {
        field: 'routeIndicatorUrl',
        eventType: 'ROUTEINDICATOR',
        type: 'route_indicator',
...
function detectFileFormat(apiData: ApiPlacemark): string {
  ...
  const hasOFCPolylines = (apiData.polylines || []).some((p) =>
    p.type?.includes('OFC'),
  );
  const avgPropertyCount =
    (apiData.points || []).reduce(
      (sum, p) => sum + (p.properties ? Object.keys(p.properties).length : 0),
      0,
    ) / (apiData.points?.length || 1);

  if (hasNullTypes && avgPropertyCount > 40)
    return 'Complex KMZ (Server Export)';
...
function normalizePolylineType(type: string): string {
  ...
  if (
    upperType.includes('BLOCK TO FPOI') ||
  ...
  if (upperType.includes('OFC')) {
    return upperType.includes('PROPOSED')
      ? 'Proposed Cable'
      : 'Incremental Cable';
  }
...
export function detectSurveyFileType(
  apiData: ApiPlacemark,
): 'physical_survey' | 'infrastructure_assets' {
  ...
  const hasPhysicalSurveyEvents = (apiData.points || []).some(
    (point) =>
      surveyEventTypes.includes(point.type) ||
      (point.properties?.event_type &&
        surveyEventTypes.includes(point.properties.event_type)),
  );
  ...
  const hasInfrastructureAssets = (apiData.points || []).some(
    (point) =>
      infrastructureTypes.includes(point.type) ||
      (point.properties?.type &&
        infrastructureTypes.includes(point.properties.type)),
  );

  // Check polylines for cable infrastructure
  const hasCableInfrastructure = (apiData.polylines || []).some(
    (polyline) =>
      polyline.type &&
  ...
  if (
    hasInfrastructureAssets &&
    (hasCableInfrastructure || !hasPhysicalSurveyEvents)
  ) {
  ...
  (apiData.points || []).forEach((point, index) => {
    try {
      ...
      if (!isValidCoordinate(lat, lng)) {
        console.warn(
          `Invalid coordinates for survey infrastructure point ${index}:`,
          { lat, lng },
        );
        return;
      }
      ...
      if (!PLACEMARK_CATEGORIES[category]) {
        console.warn(
          `Unknown survey infrastructure category: ${category}, defaulting to ASSET`,
        );
        category = 'ASSET';
      }
      ...
      processedPlacemarks.push({
        ...
        name:
          point.properties?.name || point.name || `${category} ${index + 1}`,
        category,
        type: 'point',
        coordinates: { lat, lng },
      ...
      console.error(
        `Error processing survey infrastructure point ${index}:`,
        error,
      );
    }
  });
  ...
  (apiData.polylines || []).forEach((polyline, index) => {
    try {
      if (
        ...
        console.warn(
          `Invalid polyline coordinates for survey infrastructure polyline ${index}`,
        );
        return;
      }
      ...
      if (validCoordinates.length < 2) {
        console.warn(
          `Insufficient valid coordinates for survey infrastructure polyline ${index}`,
        );
        return;
      }
      ...
      if (!PLACEMARK_CATEGORIES[category]) {
        console.warn(
          `Unknown survey infrastructure polyline category: ${category}`,
        );
      ...
      console.error(
        `Error processing survey infrastructure polyline ${index}:`,
        error,
      );
    }
  });
...
export function processApiData(apiData: ApiPlacemark): {
  ...
  const extractPointType = (point: ApiPoint): string => {
    try {
      ...
      if (
        name.includes('OLT') ||
        name.includes('Block Router') ||
        name.includes('BSNL EXCHANGE')
      ) {
        return 'Block Router';
      }
      ...
      if (
        name.includes('BHQ') ||
        name.includes('Block Headquarters') ||
        name.includes('BLOCK OFFICE')
      ) {
        return 'BHQ';
      }
  ...
  (apiData.points || []).forEach((point: ApiPoint, index: number) => {
    try {
      ...
      if (hasValidCoords) {
        ...
        if (!PLACEMARK_CATEGORIES[category]) {
          console.warn(
            `Category '${category}' not found in PLACEMARK_CATEGORIES. Using FPOI as fallback.`,
          );
  ...
  (apiData.polylines || []).forEach((polyline: ApiPolyline, index: number) => {
    try {
      ...
      if (
        ...
        console.warn(
          `Invalid polyline coordinates for polyline ${index}:`,
          polyline.coordinates,
        );
        return;
      }
      ...
      if (validCoordinates.length < 2) {
        console.warn(
          `Insufficient valid coordinates for polyline ${index}:`,
          polyline,
        );
        return;
      }
  ...
  const uniqueDirectTypes = [
    ...new Set(typeAnalysis.map((item) => item.directType)),
  ];
  const uniquePropsTypes = [
    ...new Set(typeAnalysis.map((item) => item.propertiesType)),
  ];
  const uniqueExtractedTypes = [
    ...new Set(typeAnalysis.map((item) => item.extractedType)),
  ];
  ...
  if (polylineAnalysis.length > 0) {
    ...
    const uniquePolylineTypes = [
      ...new Set(polylineAnalysis.map((item) => item.type)),
    ];
    const uniqueNormalizedTypes = [
      ...new Set(polylineAnalysis.map((item) => item.normalizedType)),
    ];
  }
  ...
  const categoryResults = Object.entries(categoryCounts)
    ...
    .sort(([_, a], [__, b]) => b - a);
  console.table(
    categoryResults.map(([name, count]) => ({ Category: name, Count: count })),
  );

  // Flag any issues
  const nullTypes = typeAnalysis.filter((item) => item.directType === 'NULL');
  const unknownCategories = typeAnalysis.filter(
    (item) => item.category === 'FPOI' && item.extractedType !== 'FPOI',
  );
...
function isValidCoordinate(lat: number, lng: number): boolean {
  return (
    ...
    lng <= 180 &&
    lat !== 0 &&
    lng !== 0 // Exclude null island unless specifically needed
  );
}
...
function getCategoryFromName(name: string): string {
  ...
  if (upperName.includes('RAIL')) return 'Railway Cross';
  if (upperName.includes('KM') || upperName.includes('KILOMETER'))
    return 'KILOMETERSTONE';
  if (upperName.includes('ROUTER') || upperName.includes('OLT'))
    return 'Block Router';
  if (upperName.includes('HIGHWAY')) return 'N Highway Cross';
  ...
  if (
    upperName.includes(' GP ') ||
    upperName.endsWith(' GP') ||
    upperName.includes('PANCHAYAT')
  ) {
    return 'GP';
  }
  ...
  if (
    upperName.includes('BHQ') ||
    upperName.includes('HEADQUARTERS') ||
    upperName.includes('BLOCK OFFICE')
  ) {
    return 'BHQ';
  }
...
export function processPhysicalSurveyData(apiData: PhysicalSurveyApiResponse): {
  ...
  Object.entries(apiData.data).forEach(([blockId, points]) => {
      ...
      try {
        ...
        if (point.gp_data && Array.isArray(point.gp_data)) {
          point.gp_data.forEach((gpItem: any, gpIndex: number) => {
            try {
              ...
              const gpCoordsParts = gpItem.gpCoordinates
                .split(',')
                .map((c: string) => c.trim());
              ...
              const poleCoordsParts = gpItem.poleCoordinates
                .split(',')
                .map((c: string) => c.trim());
              ...
              const earthPitCoordsParts = gpItem.earthPitCoordinates
                .split(',')
                .map((c: string) => c.trim());
              ...
              if (isValidCoordinate(gpLat, gpLng)) {
                categoryCounts['GP_LOCATION'] =
                  (categoryCounts['GP_LOCATION'] || 0) + 1;
                processedPlacemarks.push({
              ...
              console.error(
                `Error processing GP data item ${gpIndex} in block ${blockId}:`,
                error,
              );
            }
          });
        ...
        if (point.blk_data) {
          try {
            ...
            const bsnlCoordsParts = blockData.bsnlCordinates
              .split(',')
              .map((c: string) => c.trim());
            ...
            if (isValidCoordinate(bsnlLat, bsnlLng)) {
              ...
              const photoFields = [
                {
                  field: 'bsnlCableEntryPhoto',
                  label: 'BSNL Cable Entry Photo',
                },
                { field: 'bsnlCableExitPhoto', label: 'BSNL Cable Exit Photo' },
                {
                  field: 'bsnlExistingRackPhoto',
                  label: 'BSNL Existing Rack Photo',
                },
                { field: 'bsnlLayoutPhoto', label: 'BSNL Layout Photo' },
                {
                  field: 'bsnlProposedRackPhoto',
                  label: 'BSNL Proposed Rack Photo',
                },
              ...
              categoryCounts['BSNL_BLOCK'] =
                (categoryCounts['BSNL_BLOCK'] || 0) + 1;
              processedPlacemarks.push({
            ...
            console.error(
              `Error processing block data in block ${blockId}:`,
              error,
            );
          }
        ...
        if (!isValidCoordinate(lat, lng)) {
          console.warn(
            `Invalid coordinates for physical survey point ${blockId}-${index}:`,
            { lat, lng, point },
          );
          return;
        }
        ...
        if (point.event_type === 'LIVELOCATION') {
          ...
          const timestamp =
            parseTimestamp(point.createdTime) ||
            parseTimestamp(point.created_at);
          ...
          if (point.event_type === 'VIDEORECORD' && point.videoDetails) {
            try {
              ...
              if (vd && vd.videoUrl && vd.videoUrl.trim() !== '') {
                categoryCounts['VIDEO_SURVEY'] =
                  (categoryCounts['VIDEO_SURVEY'] || 0) + 1;
              }
            } catch (error) {
              console.warn(
                `Failed to parse videoDetails for survey ${point.survey_id}:`,
                error,
              );
            }
          }
          ...
          if (
            ...
            categoryCounts['PHOTO_SURVEY'] =
              (categoryCounts['PHOTO_SURVEY'] || 0) + 1;
          }
        }
      } catch (error) {
        console.error(
          `Error processing physical survey point ${blockId}-${index}:`,
          error,
          point,
        );
      }
    ...
    if (liveLocationPoints.length > 1) {
      ...
      const surveyGroups = liveLocationPoints.reduce(
        (groups, p) => {
      ...
      Object.entries(surveyGroups).forEach(([surveyId, routePoints]) => {
        if (routePoints.length > 1) {
          try {
            ...
            categoryCounts['SURVEY_ROUTE'] =
              (categoryCounts['SURVEY_ROUTE'] || 0) + 1;
            processedPlacemarks.push({
            ...
            console.error(
              `Error creating survey route for ${surveyId}:`,
              error,
            );
          }
        }
...
// -----------------------------
export function processDesktopPlanningData(
  apiData: DesktopPlanningApiResponse,
): {
  ...
  (apiData.data || []).forEach((network) => {
    ...
    (network.points || []).forEach((point) => {
      try {
        ...
        if (!Array.isArray(coordsArray) || coordsArray.length !== 2) {
          console.warn(
            `Invalid coordinates format for desktop point ${point.id}:`,
            point.coordinates,
          );
          return;
        }
        ...
        if (!isValidCoordinate(latitude, longitude)) {
          console.warn(
            `Invalid coordinate values for desktop point ${point.id}:`,
            { latitude, longitude },
          );
          return;
        }
        ...
        try {
          ...
          console.warn(
            `Failed to parse properties for desktop point ${point.id}:`,
            error,
          );
        }
    ...
    (network.connections || []).forEach((connection) => {
      try {
        ...
        if (!Array.isArray(coordsArray) || coordsArray.length < 2) {
          console.warn(
            `Invalid coordinates format for desktop connection ${connection.id}:`,
        ...
        if (validCoordinates.length < 2) {
          console.warn(
            `Insufficient valid coordinates for desktop connection ${connection.id}`,
          );
          return;
        }
        ...
        try {
          ...
          console.warn(
            `Failed to parse properties for desktop connection ${connection.id}:`,
            error,
          );
        }
        ...
        console.error(
          'Error processing desktop planning connection:',
          connection,
          error,
        );
      }
...
function getDesktopPlanningCategory(
  assetType: string,
  itemType: 'point' | 'polyline',
): string {
...
// -----------------------------
export function processRectificationData(
  apiData: RectificationApiResponse | null,
): {
    ...
    items.forEach((item) => {
      try {
        ...
        if (!isValidCoordinate(startLat, startLng)) {
          console.warn(
            `Invalid start coordinates for rectification item ${item.id}:`,
            { startLat, startLng },
          );
          return;
        }
        ...
        if (isPolyline) {
          ...
          if (!isValidCoordinate(endLat, endLng)) {
            console.warn(
              `Invalid end coordinates for rectification item ${item.id}:`,
              { endLat, endLng },
            );
            return;
          }
        ...
        try {
          ...
          console.warn(
            `Failed to parse images for rectification item ${item.id}:`,
            error,
          );
          imageUrls = [];
        }
...
function getRectificationCategory(workType: string): string {
  ...
  if (
    normalizedWork.includes('CHAMBER INSTALLATION') ||
  ...
  if (
    normalizedWork.includes('CLOSER REPLACEMENT') ||
  ...
  if (
    normalizedWork.includes('ROUTE UNDER ROAD') ||
  ...
  if (
    normalizedWork.includes('PATCH REPLACEMENT') ||
  ...
  if (
    normalizedWork.includes('CABLE REPAIR') ||
  ...
  if (
    normalizedWork.includes('JOINT REPAIR') ||
...
// Process Rectification data that uses Physical Survey structure
export function processRectificationSurveyData(
  apiData: PhysicalSurveyApiResponse | null,
): {
  placemarks: (
    | ProcessedPhysicalSurvey
    | ProcessedGPData
  ...
  Object.entries(apiData.data).forEach(([blockId, points]) => {
    ...
    points.forEach((point: any, index: number) => {
      try {
        ...
        if (point.gp_data && Array.isArray(point.gp_data)) {
          point.gp_data.forEach((gpItem: any, gpIndex: number) => {
            try {
                  ...
                  if (photoPath.startsWith('[') && photoPath.endsWith(']')) {
                    try {
                      ...
                      if (Array.isArray(parsedArray)) {
                        parsedArray.forEach(
                          (innerPath: string, innerIdx: number) => {
              ...
              console.error(
                `Error processing GP data in block ${blockId}:`,
                error,
              );
            }
          });
        ...
        console.error(
          `Error processing rectification point ${blockId}-${index}:`,
          error,
        );
      }
    });
    ...
    if (liveLocationPoints.length > 1) {
      ...
      Object.entries(surveyGroups).forEach(([surveyId, routePoints]) => {
        if (routePoints.length > 1) {
          ...
          categoryCounts['Rectification: SURVEY_ROUTE'] =
            (categoryCounts['Rectification: SURVEY_ROUTE'] || 0) + 1;
          processedPlacemarks.push({
...
export function processJointsData(apiData: JointsApiResponse | null): {
  ...
  const JointCategories = ['Joint: SJC', 'Joint: BJC'];
  ...
  (apiData.data || []).forEach((joint) => {
    try {
      ...
      if (!isValidCoordinate(latitude, longitude)) {
        console.warn(
          `Invalid coordinate values for Joint point ${joint.joint_name}:`,
          { latitude, longitude },
        );
        return;
      }
  ...
  const categories: PlacemarkCategory[] = JointCategories.map((name) => ({
...
export function processConstructionData(
  apiData: ConstructionApiResponse | null,
): {
  placemarks: ProcessedConstruction[];
  categories: PlacemarkCategory[];
} {
  if (!apiData || !apiData.data) {
    return { placemarks: [], categories: [] };
  ...
  const processedPlacemarks: ProcessedConstruction[] = [];
  const categoryCounts: Record<string, number> = {};
  ...
  Object.entries(apiData.data).forEach(([blockId, points]) => {
    points.filter((point) => point.status === 0).forEach((point, index) => {
      try {
        ...
        if (eventType === 'DEPTH') {
          category = 'Construction: Depth';
        } else if (eventType === 'STARTPIT') {
          category = 'Construction: Start Pit';
        } else if (eventType === 'ENDPIT') {
          category = 'Construction: End Pit';
        }
        ...
        if (point.depthLatlong && eventType === 'DEPTH') {
          const parts = point.depthLatlong.split(',').map((s) => s.trim());
          lat = parseFloat(parts[0]);
          lng = parseFloat(parts[1]);
        } else if (point.startPitLatlong && eventType === 'STARTPIT') {
          const parts = point.startPitLatlong.split(',').map((s) => s.trim());
          lat = parseFloat(parts[0]);
          lng = parseFloat(parts[1]);
        } else if (point.endPitLatlong && eventType === 'ENDPIT') {
          const parts = point.endPitLatlong.split(',').map((s) => s.trim());
          lat = parseFloat(parts[0]);
          lng = parseFloat(parts[1]);
        }
        ...
        if (!isValidCoordinate(lat, lng)) {
          console.warn(
            `Invalid coordinate for construction point ${point.id}-${point.eventType}:`,
            {
              lat,
              lng,
            },
        ...
        if (eventType === 'DEPTH') {
          photoPaths = point.depthPhoto || [];
        } else if (eventType === 'STARTPIT') {
          photoPaths = point.startPitPhotos || [];
        } else if (eventType === 'ENDPIT') {
          photoPaths = point.endPitPhotos || [];
        }
            ...
            if (photoPath && typeof photoPath === 'string') {
              ...
              if (isValidImageUrl(resolvedUrl)) {
                ...
                images.push({
                  url: resolvedUrl,
                  type: 'general',
                  label: `${eventType} Photo ${images.length + 1}`,
                });
              }
            }
          });
        ...
        processedPlacemarks.push({
          ...
          order_id: point.order_index,
          name: `${category.replace('Construction: ', '')} - ${point.survey_id}-${point.id}`,
          category,
          type: 'point',
          coordinates,
          surveyId: String(point.survey_id),
          eventType: point.eventType,
          blockId: blockId,
          images,
          hasImages: images.length > 0,
          linkName: point.link_name,
          routeBelongsTo: point.routeBelongsTo,
          roadType: point.roadType,
          cableLaidOn: point.cableLaidOn,
          soilType: point.soilType,
          depthMeters: point.depthMeters,
          roadWidth: point.roadWidth,
          distance: point.distance,
          roadMargin: point.road_margin,
          areaType: point.area_type,
        });
      } catch (error) {
        console.error('Error processing construction data:', point, error);
      }
    });
  });

  const categories: PlacemarkCategory[] = constructionCategories
    .map((name) => ({
      id: `construction-${name.toLowerCase().replace(/\s+/g, '-')}`,
      name,
      count: categoryCounts[name] || 0,
      visible: true,
      color: PLACEMARK_CATEGORIES[name]?.color || '#6B7280',
